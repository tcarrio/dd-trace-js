...
| rateLimit       | `DD_TRACE_RATE_LIMIT`              | `100`          | Global rate limit that is applied on the global sample rate and all rules. |
...
| ingestion.rateLimit  | `DD_TRACE_RATE_LIMIT`         | `100`          | Controls the ingestion rate limit between the agent and the backend. Defaults to deferring the decision to the agent. |
