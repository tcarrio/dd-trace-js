...
class TextMapPropagator {
  ...
  _injectTags (spanContext, carrier) {
    ...
    const root = trace.started[0]._spanContext

    if (this._config.tagsHeaderMaxLength === 0) {
      root._tags['_dd.propagation_error:disabled'] = 1
      return
    }
    ...
    if (header.length > this._config.tagsHeaderMaxLength) {
      root._tags['_dd.propagation_error:inject_max_size'] = 1
    } else if (tagsExpr.test(header)) {
      root._tags['_dd.propagation_error:encoding_error'] = 1
    } else {
  ...
  _extractTags (carrier, spanContext) {
    ...
    if (this._config.tagsHeaderMaxLength === 0) {
      ...
      root._tags['_dd.propagation_error:extract_max_size'] = 1
    } else if (tagsExpr.test(carrier[tagsKey])) {
      root._tags['_dd.propagation_error:decoding_error'] = 1
    } else {
